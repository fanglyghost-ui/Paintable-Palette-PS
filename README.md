...
这个版本保留了原有的可涂抹调色盘逻辑，并围绕实际绘画时的“存色、参考、混色、分区调色”重新补全了工作流。

### 4 张可保存的调色纸

面板左侧提供 1–4 共四张独立调色纸。每张调色纸都会自动保存自己的涂抹内容、导入参考和混色结果，切换时不会丢失。

垃圾桶按钮只会清空当前正在使用的调色纸，方便为不同角色、场景或光照方案分别保留调色过程。

### 将 Photoshop 选区导入调色纸

新增 **Import selection** 功能。

先在 Photoshop 中选中需要参考的图层，再框选想提取的区域，点击面板顶部的 **Import selection**，即可将该图层中选区的内容导入当前调色纸。

导入时会按照选区的实际宽高完整截取，不再受固定小尺寸限制。适合把画面中的局部配色、照片参考或已画好的区域直接放到调色纸上，再进行吸色和混色。

### 24 格常用颜色盒

面板底部新增 24 格常用颜色盒，用于保存高频使用的前景色。

- 点击 `+ Add`：将 Photoshop 当前前景色保存到第一个空位。
- 点击保存的色块：将该颜色设为 Photoshop 前景色。
- 点击 `Edit`：进入编辑模式，可拖动色块调整位置，也可点击 `×` 删除颜色。

### 更自然的颜料混色

默认混色模式改为 **Mixbox Pigment（NC）**，混合效果更接近颜料，而非普通透明叠加。

仍可切换回 RGB 模式。Mixbox 版本受 CC BY-NC 4.0 许可证约束，仅可用于非商业用途。

> 面板会使用 Photoshop 当前前景色进行涂抹与取色回写；它不会替换 Photoshop 里当前选择的笔刷或橡皮擦预设。
